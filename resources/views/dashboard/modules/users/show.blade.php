    ...
    <section class="grid gap-4 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside class="page-panel panel-gray lg:sticky lg:top-6 lg:self-start">
            ...
            <nav class="mt-4 space-y-2">
                <a href="{{ route('dashboard.modules.users') }}" class="flex items-center justify-between rounded-lg border border-[#cfcfcf] bg-[#ececec] px-3 py-2 text-sm text-[#1f1f1f] dark:border-[#343434] dark:bg-[#222222] dark:text-[#f3f3f3]" title="회원목록">
                    <span>회원목록</span>
                    <span class="text-xs">01</span>
                </a>
                <a href="{{ route('dashboard.modules.users.show', $user) }}" class="flex items-center justify-between rounded-lg border border-[#cfcfcf] bg-[#ececec] px-3 py-2 text-sm text-[#1f1f1f] dark:border-[#343434] dark:bg-[#222222] dark:text-[#f3f3f3]" title="회원상세">
                    <span>회원상세</span>
                    <span class="text-xs">02</span>
                </a>
                <a href="{{ route('dashboard.modules.users.permissions', $user) }}" class="flex items-center justify-between rounded-lg border border-[#d8d8d8] bg-[#f5f5f5] px-3 py-2 text-sm text-[#4f4f4f] transition hover:bg-[#ededed] hover:text-[#1f1f1f] dark:border-[#2a2a2a] dark:bg-[#1a1a1a] dark:text-[#b9bbc0] dark:hover:bg-[#222222] dark:hover:text-[#d6d6dd]" title="권한관리">
                    <span>권한관리</span>
        ...
        <div class="space-y-4">
            ...
            <section class="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
                <div class="page-panel">
                    ...
                    @if ($canManageUser)
                        <form method="POST" action="{{ route('dashboard.modules.users.role.update', $user) }}" class="mt-4 space-y-4">
                            @csrf
                            @method('PATCH')
                            <div>
                                <label for="role" class="text-sm font-medium text-gray-900 dark:text-gray-100">Role 변경</label>
                                <select id="role" name="role" title="Role 변경" class="input-field mt-2">
                        ...
                        <form method="POST" action="{{ route('dashboard.modules.users.status.update', $user) }}" class="mt-6 space-y-4">
                            @csrf
                            @method('PATCH')
                            <div>
                                <label for="status" class="text-sm font-medium text-gray-900 dark:text-gray-100">상태 변경</label>
                                <select id="status" name="status" title="상태 변경" class="input-field mt-2">
                ...
                <div class="page-panel panel-gray">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <a href="{{ route('dashboard.modules.users.permissions', $user) }}" class="btn-secondary" title="권한관리 이동">권한관리</a>
                    </div>
