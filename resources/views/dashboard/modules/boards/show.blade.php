    ...
    <section class="grid gap-4 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside class="page-panel panel-gray lg:sticky lg:top-6 lg:self-start">
            ...
            <nav class="mt-4 space-y-2">
                ...
                <a href="{{ route('dashboard.modules.boards.show', $board) }}" class="flex items-center justify-between rounded-lg border border-[#cfcfcf] bg-[#ececec] px-3 py-2 text-sm text-[#1f1f1f] dark:border-[#343434] dark:bg-[#222222] dark:text-[#f3f3f3]" title="게시글 상세">
                    <span>게시글 상세</span>
                    <span class="text-xs">02</span>
                </a>
                @if ($canUpdate)
                    <a href="{{ route('dashboard.modules.boards.edit', $board) }}" class="flex items-center justify-between rounded-lg border border-[#d8d8d8] bg-[#f5f5f5] px-3 py-2 text-sm text-[#4f4f4f] transition hover:bg-[#ededed] hover:text-[#1f1f1f] dark:border-[#2a2a2a] dark:bg-[#1a1a1a] dark:text-[#b9bbc0] dark:hover:bg-[#222222] dark:hover:text-[#d6d6dd]" title="게시글 수정">
                        <span>게시글 수정</span>
        ...
        <div class="space-y-4">
            ...
            <section class="page-panel panel-gray">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('dashboard.modules.boards') }}" class="btn-secondary" title="게시글 목록">목록</a>
                    @if ($canUpdate)
                        <a href="{{ route('dashboard.modules.boards.edit', $board) }}" class="btn-primary" title="게시글 수정">수정</a>
                    @endif
                    @if ($canDelete)
                        <form method="POST" action="{{ route('dashboard.modules.boards.destroy', $board) }}">
                            @csrf
                            @method('DELETE')
