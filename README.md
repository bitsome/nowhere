...
공항·셔틀 운행 마켓 — 운행을 **등록**하고, 다른 드라이버가 **가져와서**(claim) 운행·정산하는 2면 시장 플랫폼.

```
등록자 ──운행 등록──▶ 마켓(공개) ──claim──▶ 수행자 ──운행·완료──▶ 리뷰·평점·정산
```

## 주요 기능

- **운행 워크스페이스**: 단일/셋트 운행 등록, AI 구조화, 마켓 노출, claim, 상태 전환(published → trading → accepted → driving → completed → settled), 일괄 정산
- **평점·리뷰**: 완료 운행에서 상호 리뷰, 프로필에 평점·완료 실적·누적 매출 표시
- **채팅**: 운행 연동 대화, 이미지 첨부, 실시간 안읽음 배지
- **커뮤니티**: 게시글·이미지·댓글·좋아요
...
| 영역 | 스택 |
|---|---|
| 백엔드 | Laravel 13, Sanctum, SQLite(개발) |
| 프론트엔드 | Vue 3, Vite, Pinia, Vue Router, Naive UI (독립 SPA `frontend/`) |
| 실시간 | SSE `/api/events` (once 모드: 상태 스냅샷 후 종료·5초 재연결) |
| 테스트 | Pest(PHPUnit) — 210+ 테스트 케이스 |
...
php artisan migrate --seed   # 데모 계정 + 운행·채팅·커뮤니티 시드
...
데모 계정(로컬 시드 기준): `test@example.com` / `password` (Operator), `market@example.com`(Admin), `driver01@example.com`(Driver) — 서버 배포본은 전 계정 비밀번호 `123456`로 통일된다.

## 테스트

```bash
php artisan test   # 전체 210건 (운행·리뷰·채팅·커뮤니티·통계·추천·인증·관리자)
```
...
frontend/
  src/views/              # SPA 화면 (마켓·내 운행·채팅·커뮤니티·대시보드·알림·프로필)
  src/components/         # 운행 카드·채팅·헤더 배지·스켈레톤
