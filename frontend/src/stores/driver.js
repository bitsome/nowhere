...
import { apiMyDriver, apiSetDriverMatchEnabled, apiSetDriverStatus } from '../api/driver';
    ...
    actions: {
        async load() {
            try {
                ...
                this.matchEnabled = driver.match_enabled ?? true;
                this.loaded = true;
        ...
        async setMatchEnabled(enabled) {
            const { data } = await apiSetDriverMatchEnabled(enabled);
            this.matchEnabled = Boolean(data.data.match_enabled);
