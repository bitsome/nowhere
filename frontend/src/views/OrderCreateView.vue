...
import SegmentedGroup from '../components/common/SegmentedGroup.vue';
...
<template>
    <div>
        ...
        <template v-if="screen === 'list'">
            <div class="page-head">
                <div>
                    <p class="page-head__desc">가져온 운행만 날짜별로 모아 보여줍니다. 등록 운행은 프로필의 내 마켓에서 관리합니다.</p>
                </div>
            ...
            <div class="schedule-view">
                <SegmentedGroup size="large" stretch>
                    <n-radio-group v-model:value="viewMode" size="small">
                        <n-radio-button value="list">목록</n-radio-button>
                        <n-radio-button value="calendar">캘린더</n-radio-button>
                    </n-radio-group>
                </SegmentedGroup>
            </div>

            <!-- 보기 단위 전환 — 일별 / 월별 / 연별 (목록 모드에서만 표시) -->
            <div v-if="viewMode === 'list'" class="schedule-unit">
                <SegmentedGroup size="small" stretch>
                    <n-radio-group v-model:value="groupUnit" size="small" @update:value="setGroupUnit">
                        <n-radio-button v-for="unit in GROUP_UNITS" :key="unit.value" :value="unit.value">
                            {{ unit.label }}
                        </n-radio-button>
                    </n-radio-group>
                </SegmentedGroup>
            ...
            <div v-if="viewMode === 'calendar'" class="cal">
                ...
                <div class="cal__grid">
                    <div v-for="dow in ['일', '월', '화', '수', '목', '금', '토']" :key="dow" class="cal__dow">{{ dow }}</div>
                    <button
...
/* 묶음 단위 전환 — 일별 / 월별 / 연별 (n-radio 버튼 그룹 스타일) */
.schedule-unit {
    margin-bottom: 10px;
}
.schedule-unit :deep(.n-radio-group) {
    display: flex;
    width: 100%;
}
.schedule-unit :deep(.n-radio-button) {
    flex: 1;
...
/* ── 목록/캘린더 보기 전환 (n-radio 버튼 그룹 스타일) ── */
.schedule-view {
    margin-bottom: 10px;
}
.schedule-view :deep(.n-radio-group) {
    display: flex;
    width: 100%;
}
.schedule-view :deep(.n-radio-button) {
    flex: 1;
    justify-content: center;
}
